...
public class QueryService {
  ...
  private boolean isCalciteFallbackAllowed() {
    if (settings != null) {
      Boolean fallbackAllowed = settings.getSettingValue(Settings.Key.CALCITE_FALLBACK_ALLOWED);
      return fallbackAllowed != null ? fallbackAllowed : false; // Default to true if not set
    } else {
      return true;
    }
  }
  ...
  private boolean isCalciteEnabled(Settings settings) {
    if (settings != null) {
      Boolean calciteEnabled = settings.getSettingValue(Settings.Key.CALCITE_ENGINE_ENABLED);
      return calciteEnabled != null ? calciteEnabled : false; // Default to false if not set
    } else {
      return false;
    }
  }
